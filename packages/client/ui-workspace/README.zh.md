...
Session 行菜单还提供**关注**／**取消关注**。被关注的会话移入渲染在所有工作区上方（单列表模式下则在列表上方）的**置顶**区，按关注时间从新到旧排列；置顶区为空时不显示。关注是浏览器本地的纯投影：每个顺序记账和 Host 工作区成员席位都原样保留，因此取消关注会让行回到原来的位置，状态筛选在置顶区内同样生效。无论采用哪种 Session 顺序，Workspace 拖拽顺序都由 Host 持久化。
