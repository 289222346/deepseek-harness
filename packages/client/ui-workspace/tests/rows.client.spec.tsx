...
describe('workspace browser rows', () => {
  it('omits only an empty leading status slot in the hierarchy-free flat list', () => {
    const idle: SessionNode = {
      ...
      runningSubagentCount: 0, pinned: false, completed: false, updatedAt: 0,
    }
    ...
    const renderRow = (over: Partial<SessionNode>) => render(
      <SessionNodeItem
        node={{
          ...
          runningSubagentCount: 0, pinned: false, completed: false, updatedAt: 0, ...over,
        }}
  ...
  it('shows descendant activity without describing an idle parent as running', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 2, pinned: false, completed: false, updatedAt: 0,
      }
  ...
  it('keeps descendant activity secondary while the parent is running', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 1, pinned: false, completed: false, updatedAt: 0,
      }
      render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onPin={vi.fn()} t={t} />)
      const row = screen.getByRole('treeitem')
  ...
  it('keeps child activity as a secondary status while user attention is primary', () => {
    const node: SessionNode = {
      ...
      running: false, runningSubagentCount: 1, pinned: false, completed: false, updatedAt: 0,
    }
    render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onPin={vi.fn()} t={t} />)
    const row = screen.getByRole('treeitem')
  ...
  it('session row menu opens without opening the session and dispatches rename, fork, pin, and archive', () => {
    ...
    // Pin dispatches with the toggle; a pinned row offers the inverse label.
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
    ...
    view.rerender(<SessionNodeItem node={{ ...node, pinned: true }} currentId={undefined} now={0}
      onOpen={onOpen} onRename={onRename} onFork={onFork} onArchive={onArchive} onPin={onPin} t={t} />)
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
  ...
  it.each([
    ...
    try {
      const node: SessionNode = {
        ...
        pendingInteraction, running: true, runningSubagentCount: 0, pinned: false, completed: false, updatedAt: 0,
      }
  ...
  it('completed hover card shows the Completed status line', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 0, pinned: false, completed: true, updatedAt: 0,
      }
      render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onPin={vi.fn()} t={t} />)
      fireEvent.pointerEnter(screen.getByRole('treeitem').parentElement as HTMLElement)
